...
namespace Cloverwatch {
    ...
    static inline void circular_increment(uint16_t& val, const uint16_t &max) {
        ++val;
        val *= (val != max);
    }
    ...
    std::optional<T> CQueue_concurrent_SPSC<T, capacity>::pop() {
        ...
        uint16_t tail_copy = tail.load();
        uint16_t next_tail = tail.load();
    ...
    bool CQueue_concurrent_SPSC<T, capacity>::push(T&& val) {
        ...
        uint16_t head_copy = head.load();
        uint16_t next_head = head.load();
    ...
    template<typename T, uint16_t capacity>
    inline uint16_t CQueue_concurrent_SPSC<T, capacity>::remaining_capacity() const {
    ...
    template<typename T, uint16_t capacity>
    inline uint16_t CQueue_concurrent_SPSC<T, capacity>::size() const {
        uint16_t head_copy = head.load();
        uint16_t tail_copy = tail.load();
    ...
    template <uint16_t capacity>
    inline uint16_t CByteQueue_SPSC<capacity>::remaining_capacity() const {
        return ring_buf_space_get(&ring_buffer);
    }

    template<uint16_t capacity>
    uint16_t CByteQueue_SPSC<capacity>::size() const {
