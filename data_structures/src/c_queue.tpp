...
namespace Cloverwatch {
    ...
    bool CByteQueue_SPSC<capacity>::push(ReadVector<Byte> bytes) {
        ...
        if (remaining_capacity() < bytes.ref.len()) return false;

        ring_buf_put(&ring_buffer, bytes.ref.data(), bytes.ref.len());
    ...
    inline void CByteQueue_SPSC<capacity>::pop(WriteVector<Byte> buffer) {
        ...
        buffer.ref.set_len(ring_buf_get(&ring_buffer, buffer.ref.data(), buffer.ref.capacity()));
    }
